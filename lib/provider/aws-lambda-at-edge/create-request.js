...
function requestHeaders(event) {
  const eventHeaders = event.Records[0].cf.request.headers;
  
  return Object.keys(eventHeaders).reduce((headers, key) => {
    headers[key.toLowerCase()] = eventHeaders[key][0].value;
    return headers;
  }, {});
}
...
function requestUrl(event) {
  return URL.format({
    pathname: event.Records[0].cf.request.uri,
    search: event.Records[0].cf.request.querystring,
  });
}
